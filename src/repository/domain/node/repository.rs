...
impl Repository {
    pub fn node_find_all(&self) -> Result<Vec<Node>, RepositoryError> {
        let mut statement = self.conn.prepare(&format!("SELECT * FROM {}", TABLE_NAME))?;
        let result: Result<Vec<Node>, _> = from_rows::<Node>(statement.query([])?).into_iter().collect();

        result.map_err(Into::into)
    }

    pub fn node_get_by_key(&self, key: u32) -> Result<Option<Node>, RepositoryError> {
        let mut statement = self
            .conn
            .prepare(&format!("SELECT * FROM {} WHERE key = :key", TABLE_NAME))?;
    ...
    pub fn node_upsert(&self, node: Node) -> Result<(), RepositoryError> {
        ...
        match statement.execute(to_params_named(&node)?.to_slice().as_slice()) {
            Ok(_) => Ok(()),
            Err(e) => Err(e.into()),
        }
    }

    pub fn node_remove(&self, key: u32) -> Result<(), RepositoryError> {
        let mut statement = self
            .conn
            .prepare(&format!("DELETE FROM {} WHERE key = :key", TABLE_NAME))?;

        match statement.execute(params![key]) {
            Ok(_) => Ok(()),
            Err(e) => Err(e.into()),
        }
    }
